...
@RestController
@RequestMapping(path = {"/api/tresource",}, consumes = {"application/json","application/x-www-form-urlencoded"},
		produces = MediaType.APPLICATION_JSON_VALUE)
public class TenantAdminResourceController extends MBaseController<SysTenantRoleResourceDto> {
...
//	}
	...
	public List<CommonTreeWrap> treeAllMenus(@RequestParam String tid) {
		...
		List<MenuTenantDto> menus = roleResourceService.listAllMenusForGrant(Sets.newHashSet(role.getId()), tid);
		return loopMenus(menus);
	}
	...
	public List<ButtonTenantDto> listAllButtonsByMenuForGrant(@RequestParam String tid, @RequestParam String menuId) {
		...
		List<ButtonTenantDto> buttons =  roleResourceService.listPermButtonsByRolesAndMenu(Sets.newHashSet(role.getId()), menuId, tid);
		return buttons;
	}
