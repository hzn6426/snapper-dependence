...
@RestController
@RequestMapping(path = {"/eapi/group"}, consumes = {"application/json", "application/x-www-form-urlencoded"}, produces = MediaType.APPLICATION_JSON_VALUE)
public class TenantGroupController extends MBaseController<SysTenantGroupDto> {
...
//	private RedisTemplate<String, String> redisTemplate;
	...
	public List<SysTenantUserGroupDto> userDepartments(@RequestParam String tid) {
		...
		// 获取用户当前的组织
//		String group = ObjectUtil.toStringIfNotNull(redisTemplate.opsForHash().get(MessageFormat.format(RedisKeyConsts.KEY_USER_CONTEXT, currentUserName()), RedisKeyConsts.KEY_USER_GROUP_ID));
...
//		userGroupService.changeUser2Group(tid, currentUserId(), groupId);
	}
...
//	public List<GroupDto> listByParent(@RequestParam String pid){
//		return groupService.listByParent(pid);
//	}
...
//		groupDto.setParentId(TenantGroupConsts.ROOT_GROUP);
...
//	/**
//	 * 根据父组织获取子组织及用户以树的方式展示
//	 *
//	 * @param gid 父组织ID
//	 * @Return: com.baomibing.authority.dto.GroupTreeNodeDto
//	 */
//	@GetMapping("/treeGroupsAndUsersByParent")
//	public List<CommonTreeWrap> treeGroupsAndUsersByParent(@RequestParam String gid){
//		WrapGroupUserDto wraper = groupService.wrapGroupsAndUsersByParent(gid);
//		if (Checker.BeNull(wraper)) {
//			return Lists.newArrayList();
//		}
//		List<CommonTreeWrap> lists = Lists.newArrayList();
//		wraper.getChildGroups().forEach(g -> lists.add(new CommonTreeWrap().setIsLeaf(false).setParentId(g.getParentId())
//				.setKey(g.getId()).setTitle(g.getGroupName()).setTag(UserGroupEnum.GROUP.name())));
//		wraper.getUsers().forEach(u -> lists.add(new CommonTreeWrap().setIsLeaf(true).setTag(UserGroupEnum.USER.name())
//				.setKey(u.getId()).setTitle(u.getUserRealCnName())));
//		return lists;
//	}
