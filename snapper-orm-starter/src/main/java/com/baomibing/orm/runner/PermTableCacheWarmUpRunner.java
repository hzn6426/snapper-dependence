...
//@Component
@Slf4j
public class PermTableCacheWarmUpRunner implements CommandLineRunner {

    @Value("${spring.application.name}") private String serviceName;
    @Value("${snapper.action.package}") private String actionPackage;
...
//            }
            ...
            Set<Method> methods =
                    reflections.get(Scanners.MethodsAnnotated.with(Action.class).as(Method.class));
            ...
            for(Method method : methods) {
                ...
                ActionConnect actionConnect = method.getAnnotation(ActionConnect.class);
            ...
            String cacheActionConnectKey = RedisKeyConstant.CACHE_ACTION_CONNECT_KEY_PREFIX + serviceName;
            List<String> keys = cacheService.lRange(cacheActionConnectKey, 0 ,-1);
            if (Checker.beNotEmpty(keys)) {
                cacheService.del(keys);
            }
            actions.forEach(a -> {
                String key = RedisKeyConstant.CACHE_ACTION_CONNECT_PREFIX + a.getAction();
